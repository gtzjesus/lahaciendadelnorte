...
const HeroSection: FC<HeroSectionProps> = ({ className = '' }) => {
  return (
    <div
      className={`absolute inset-0 flex flex-col justify-between items-center py-10 ${className} z-20`}
    >
      {/* Optional red tint behind text, above overlay */}
      {/* <div className="absolute inset-0 bg-flag-red opacity-50 -z-10" /> */}

      {/* Title near the top */}
      <h1 className="uppercase font-bold text-6xl lg:text-8xl text-white leading-tight text-center px-4 mt-10">
        Build your <br /> Custom Storage
      </h1>

      {/* Buttons near the bottom */}
      <div className="flex justify-center gap-4 w-full max-w-lg px-4 mb-10">
        <Link
          href="/"
